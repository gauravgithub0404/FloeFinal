...
export class GitleaksScanner {
  ...
  public async scan(files: GeneratedFile[]): Promise<EvaluationExecutionResult> {
    ...
    const secretRules = [
      ...
      {
        id: 'gitleaks.postgres-connection-string',
      ...
      {
        id: 'gitleaks.jwt-secret-hardcoded',
    ...
    const rawStr = JSON.stringify(gitleaksArtifact);
    let hashNum = 0;
